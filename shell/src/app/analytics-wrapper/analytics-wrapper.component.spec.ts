...
describe('AnalyticsWrapperComponent', () => {
  ...
  it('appends the mfe-analytics element and clears loading on a successful load', fakeAsync(() => {
    ...
    try {
      ...
      expect(component.error()).toBe('');
      expect(component.container().nativeElement.querySelector('mfe-analytics')).not.toBeNull();
    } finally {
      stubs.restore();
      discardPeriodicTasks();
    }
  ...
  it('removes the mfe-analytics element when destroyed after a successful load', fakeAsync(() => {
    ...
    try {
      ...
      expect(containerEl.querySelector('mfe-analytics')).not.toBeNull();

      fixture.destroy();
      expect(containerEl.querySelector('mfe-analytics')).toBeNull();
    } finally {
      stubs.restore();
      discardPeriodicTasks();
    }
  ...
  it('sets the error signal and clears loading when the exposed module lookup fails', fakeAsync(() => {
    ...
    try {
      ...
      expect(component.cargando()).toBeFalse();
      expect(component.error()).toContain('Analytics');
      expect(component.container().nativeElement.querySelector('mfe-analytics')).toBeNull();
    } finally {
      stubs.restore();
      discardPeriodicTasks();
    }
